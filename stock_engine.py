...
# -------------------------------------------------------
# Detect run mode (MORNING = 08:30, EOD = 15:30 IST)
# -------------------------------------------------------
RUN_MODE = os.getenv("RUN_MODE", "MORNING").upper()
...
for symbol in stocks:
    try:
        ...
        # MORNING ONLY: Technical + Volatility
        if RUN_MODE == "MORNING":
            ...
            df_tech.append([
                symbol,
                rsi_val, macd_line.iloc[-1], macd_line.iloc[-2],
            ...
            df_vol.append([
                symbol, weekly_stddev, weekly_atr,
                weekly_wow, trend_13w
            ])
...
# -------------------------------------------------------
# SAVE OUTPUTS WITH TIME LABELS
# -------------------------------------------------------
if RUN_MODE == "EOD":
    file_fund = "output/fundamentals_1530.csv"
    pd.DataFrame(df_fund, columns=[
        "Symbol","Latest_Close","Price_1D",
        "Price_1W","Price_1M","Price_3M","Price_6M","Price_1Y",
        "Return_1D","Return_1W","Return_1M",
        "Return_3M","Return_6M","Return_1Y"
    ]).to_csv(file_fund, index=False)
    print(f"✔ Saved EOD fundamentals → {file_fund}")

else:
    file_fund = "output/fundamentals_0830.csv"
    file_tech = "output/technicals_0830.csv"
    file_vol  = "output/volatility_0830.csv"
    ...
    pd.DataFrame(df_tech, columns=[
        "Symbol","RSI","MACD_Latest","MACD_1D",
        "Signal_Latest","Signal_1D","Histogram_Trend",
        "EMA20","EMA50","EMA100",
        "Price_1W","Price_1M","Price_3M"
    ]).to_csv(file_tech, index=False)

    pd.DataFrame(df_vol, columns=[
        "Symbol","Weekly_StdDev","Weekly_ATR","Weekly_WoW","Trend_13W"
    ]).to_csv(file_vol, index=False)

    print(f"✔ Saved MORNING files → {file_fund}, {file_tech}, {file_vol}")
