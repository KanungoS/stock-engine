...
def fetch_data(symbol):
    # Method 1 — normal yfinance download
    try:
    ...
    return pd.DataFrame()  # final fallback failure
...
for symbol in stocks:
    try:
        ...
        # FIX: yfinance sometimes returns 2-D columns; squeeze to 1-D
        ...
        # ---------- Returns ----------
        return_1D = (latest - close.iloc[-2]) / close.iloc[-2] * 100
        return_1W = (latest - close.iloc[-5]) / close.iloc[-5] * 100 if len(close) >= 5 else np.nan
        return_1M = (latest - close.iloc[-22]) / close.iloc[-22] * 100 if len(close) >= 22 else np.nan
        return_3M = (latest - close.iloc[-66]) / close.iloc[-66] * 100 if len(close) >= 66 else np.nan
        return_6M = (latest - close.iloc[-132]) / close.iloc[-132] * 100 if len(close) >= 132 else np.nan
        return_1Y = (latest - close.iloc[-252]) / close.iloc[-252] * 100 if len(close) >= 252 else np.nan
        ...
        weekly_atr = (high - low).resample("W").mean().iloc[-1]  # FIXED ATR
...
print("\n🎉 SUCCESS — All 3 CSV files generated with identical stock order and correct data!\n")
