...
# -------------------------------------------------------
# LOAD & CLEAN STOCK LIST
# -------------------------------------------------------
stocks = pd.read_csv("stocks.csv")["Symbol"].tolist()
stocks = [s.strip().upper() for s in stocks]    # Removes spaces, \n, \r etc.
...
for symbol in stocks:
    try:
        ...
        # If still empty → skip
        if data is None or data.empty:
            print(f"⚠️  WARNING: No data for {symbol} — skipping")
            continue
...
print("\n🎉 All 3 CSV files generated successfully!\n")
