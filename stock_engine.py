...
# SAFE RSI (Linux-friendly, 1D only)
...
# ------------------------------------------------------------
# 13 Fridays for Volatility Window
# ------------------------------------------------------------
...
for symbol in stocks:
    ...
    def ret(p):
        if p and price_latest:
            return (price_latest - p) / p * 100
        return np.nan
    ...
    series = pd.Series(weekly_prices)

    weekly_wow = ((series.iloc[-1] - series.iloc[-2]) / series.iloc[-2]) * 100
    weekly_stddev = series.pct_change().std() * 100
    weekly_atr = (series.diff().abs().mean() / series.mean()) * 100
    trend_13w = series.diff().sum() / series.mean()
...
vol_cols = [
    "Symbol","Weekly_StdDev_Pct","Weekly_ATR_Pct","Weekly_WoW_Pct","Weekly_Trend_13W"
]
...
pd.DataFrame(fundamental_rows, columns=fund_cols).to_csv("fundamental_scores.csv", index=False)
pd.DataFrame(technical_rows, columns=tech_cols).to_csv("technical_scores.csv", index=False)
pd.DataFrame(volatility_rows, columns=vol_cols).to_csv("weekly_volatility.csv", index=False)
pd.DataFrame(combined_rows, columns=combined_cols).to_csv("master_scores.csv", index=False)
